...
- (void)configToolView {
    ...
    self.toolView.frame = CGRectMake(0, 30, self.frame.size.width, 44);
    ...
    [saveBtn addTarget:self action:@selector(cancelBtnClick) forControlEvents:UIControlEventTouchUpInside];
   // [self.toolView addSubview:saveBtn];
    
    UIButton *cancelBtn = [[UIButton alloc] init];
    cancelBtn.frame = CGRectMake(self.frame.size.width - 50, 2, 40, 40);
    [cancelBtn setImage:[UIImage imageNamed:@"icon_revocation1"] forState:UIControlStateNormal];
    [cancelBtn addTarget:self action:@selector(saveBtnClick) forControlEvents:UIControlEventTouchUpInside];
...
- (void)configPickerView {
    self.pickerView = [[UIPickerView alloc] initWithFrame:CGRectMake(0, CGRectGetMaxY(self.toolView.frame), self.frame.size.width, self.frame.size.height - 44 - 30 - 48)];
...
//    [self.pickerView selectRow:self.minuteArr.count / 2 inComponent:4 animated:YES];
    ...
    /// 刷新日
    [self.pickerView.superview bringSubviewToFront:self.pickerView];
    [self refreshDay];
}
