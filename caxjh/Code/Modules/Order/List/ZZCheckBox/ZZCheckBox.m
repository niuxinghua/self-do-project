...
//    }
    ...
    for (NSInteger i = 0; i < count; i++) {
        ...
        if ([_dataSource respondsToSelector:@selector(checkBox:titleForCheckBoxAtIndex:)]) {
            NSAttributedString *titleString = [_dataSource checkBox:self titleForCheckBoxAtIndex:i];
            if (titleString.length) {
                [button setAttributedTitle:titleString forState:UIControlStateNormal];
                [button setAttributedTitle:titleString forState:UIControlStateSelected];
                [button setAttributedTitle:titleString forState:UIControlStateHighlighted];
            }
        }
